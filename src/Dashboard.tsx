...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(roomsRef, (snapshot) => {
      ...
      if (data) {
        // Convert object { "101": {...}, "102": {...} } to Array
        const roomList = Object.entries(data).map(([id, values]) => ({
          id,
          ...values,
        }));
        setRooms(roomList);
  ...
  // Helper to delete a room (Optional admin feature)
  const handleDeleteRoom = (e, roomId) => {
    e.preventDefault(); // Prevent Link navigation
    if (window.confirm(`Are you sure you want to delete Room ${roomId}?`)) {
      remove(ref(db, `classrooms/${roomId}`));
    }
  };

  // Calculate Stats
  const totalRooms = rooms.length;
  const occupiedRooms = rooms.filter((r) => r.occupancyCount > 0).length;
  const freeRooms = totalRooms - occupiedRooms;

  const getStatusColor = (room) => {
    if (room.currentStatus === "occupied" || room.occupancyCount > 0) return "bg-red-100 text-red-700 border-red-200";
    if (room.nextClass === "faculty_coming") return "bg-amber-100 text-amber-700 border-amber-200";
    return "bg-green-100 text-green-700 border-green-200";
  };

  const getStatusDot = (room) => {
    if (room.currentStatus === "occupied" || room.occupancyCount > 0) return "bg-red-500";
        ...
        <div className="max-w-6xl mx-auto px-6 mt-6 animate-fade-in-down">
          <div className="bg-white p-6 rounded-2xl shadow-lg border-2 border-purple-100">
            ...
            <form onSubmit={handleAddRoom} className="flex gap-4">
              ...
              <button
                type="submit"
                className="px-8 py-3 bg-green-500 hover:bg-green-600 text-white font-bold rounded-xl shadow-sm transition-colors"
              >
                Save Room
              </button>
            </form>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {rooms.map((room) => (
              <Link
                to={`/room/${room.id}`}
                key={room.id}
                className="group block relative bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100"
              >
                {/* Status Banner */}
                <div className={`px-6 py-3 flex justify-between items-center border-b ${getStatusColor(room)}`}>
                  <span className="font-bold text-sm uppercase tracking-wider">
                    {room.occupancyCount > 0 ? "Occupied" : "Free"}
                  </span>
                ...
                <button 
                  ...
                  className="absolute top-14 right-4 text-gray-300 hover:text-red-500 p-2 transition-colors opacity-0 group-hover:opacity-100"
                  title="Delete Room"
                >
                  🗑️
                </button>
